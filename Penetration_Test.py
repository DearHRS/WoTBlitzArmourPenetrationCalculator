...
def e_armor_fn(n_armor, angle, normalization = 0):                   #function for effective armor after normalization
    if normalization != 0:
        if float(angle) - normalization > 0:
            e_armor = round(float(n_armor) / cos((float(angle) - normalization) * pi / 180), 3)
            return e_armor
        else:
            e_armor = round(float(n_armor) / cos((0) * pi / 180), 3)
            return e_armor
    else:
        e_armor = round(float(n_armor) / cos((float(angle)) * pi / 180), 3)
        return e_armor
...
def penetration_calculator(caliber, shell, penetration):
    if shell == 'ap' or shell == 'ap+':
        ...
        auto_ricochet = 1000  #he/hesh/hep
    if shell == 'ap' or shell == 'apcr' or shell == 'ap+' or shell == 'apcr+':                               #ap apcr shell penetration tests
        if float(angle) >= auto_ricochet:                                                                    #entry angle 70* or more test
            if float(caliber) > 3 * float(n_armor):                                                            #3 caliber's rule test
            ...
            if float(caliber) > 2 * float(n_armor):
        ...
        if float(angle) >= auto_ricochet:
            ...
            e_armor = e_armor_fn(n_armor, angle)                                                                  #effective armor against heat/he
            return penetration_rng_fn(penetration, e_armor)
    else:
        e_armor = e_armor_fn(n_armor, angle)                                                                      #effective armor against heat/he
        return penetration_rng_fn(penetration, e_armor)
...
#all the data
Tank_data = ['8',
             'lt', 'T-54 ltwt.,100,ap,175,apcr,235', 'T49 (the normal one),152,heat,152,he,75', 'T49 (the budget ru 251 gun one),90,ap,173,heat,250', 'FV301,83.4,ap,226,apcr,258', 'leKpz M 41 90 mm,90,ap,182,heat,250', 'Spähpanzer Ru 251,90,ap,180,heat,250', 'AMX 13 90,90,ap,170,apcr,248',
             'mt', 'STG,100,ap,190,apcr,260', 'T-54 first prototype,100,ap,175,apcr,235', 'T-44-100,100,ap,175,apcr,235', 'T-44 (100mm),100,ap,175,apcr,235', 'T-44 (stupid 122mm),122,ap,175,apcr,217', 'T25 Pilot Number 1,90,ap,192,apcr,243', 'T95E2,90,ap,173,heat,268', 'T69,90,ap,180,heat,250', 'T26E4 SuperPershing,90,ap,205,apcr,258', 'M26 Pershing,90,ap,160,apcr,243', 'Centurion Mk. 5/1 RAAC,83.4,ap,226,apcr,258', 'Defender Mk. 1,83.4,ap,226,apcr,258', 'Chimera,127,ap,218,apcr,252', 'Centurion Mk. I,83.4,ap,226,apcr,258', 'O-47,90,ap,190,heat,260', 'STA-1,90,ap,218.7,heat,275', 'Panzer 58,90,ap,212,apcr,259', 'Panther mit 8.8 cm L/71,90,ap,203,apcr,237', 'Indien-Panzer,90,ap,212,apcr,259', 'Panther II,90,ap,203,apcr,237', 'M4A1 Revalorisé,105,apcr,200,heat,250', 'Lorraine 40 t,90,ap,212,apcr,259', 'FCM 50 t,90,ap,212,apcr,259', 'AMX Chasseur de chars,90,ap,212,apcr,259', 'Progetto M35 mod. 46,90,ap,180,apcr,259', 'P.44 Pantera,90,ap,203,apcr,259', 'T-34-3,122,ap,175,heat,270', '59-Patton,90,ap,180,heat,268', 'T-34-2 (stupid 100mm),100,ap,181,apcr,241', 'T-34-2 (122mm),122,ap,175,heat,250', 'Type 59,100,ap,181,apcr,241',
             'ht',
             'td',
             '9',
             'lt', 'T92E1,152,apcr,210,heat,310', 'Bat.-Châtillon 25t AP,100,ap,232,apcr,263', 'Vickers Cruiser,105,apcr,240,heat,290',
             ...
             '10',
             'lt', 'Bat.-Châtillon 25 t,105,apcr,240,heat,295', 'Vickers Light,105,apcr,240,heat,290', 'XM551 Sheridan,152,apcr,230,heat,310',
             ...
             'td', 'Object 263,130,ap,290,apcr,330', 'Object 268,152,ap,303,heat,380', 'T110E3,155,ap,295,apcr,375', 'T110E4,155,ap,295,apcr,375', 'FV 4005,123,ap,280,apcr,370', 'FV 217 Badger,123,ap,280,apcr,370', 'FV 215b (183),183,hesh,220,ap,310', 'Ho-Ri Type III,150,ap,310,ap+,380', 'Grille 15,150,ap,279,heat,334', 'Jagdpanzer E 100,170,ap,299,heat,380', 'AMX 50 Foch (155),155,ap,280,heat,370', 'WZ-113G FT,152,ap,290,heat,375',
...
while 1:
    ...
    if tank_type == 'all':                  #armor test against all tank types
        lt_failed_count = 0                 #count for failed penetration attempts by light tanks
        mt_failed_count = 0                 #count for failed penetration attempts by medium tanks
        ht_failed_count = 0                 #count for failed penetration attempts by heavy tanks
        td_failed_count = 0                 #count for failed penetration attempts by tank destroyers
    ...
    if failed_count == 0:                                                                     # Printing if all can penetrate
        ...
        if tank_type == 'all':
            ...
            if failed_count == total_tanks:
                print()
                print('none of them can penetrate')
            else:
                print()
