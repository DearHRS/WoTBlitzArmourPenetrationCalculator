...
while 1:
    ...
    copy = Tank_data[:]                     #copy of Tank data, for changing
    x = copy.index(tier)                    #searching tier
    copy[x] = 'placer'                      #changing copy
    data_= copy[x + 1 : copy.index(tier)]   #obtaining the requested part of tank data
    ...
    if tank_type == 'all':                  #armor test against all tank types
        for i in range(0, len(data_)):
            if data_[i] == 'lt':
                ...
                name_of_tank, caliber, s_shell, s_penetration, p_shell, p_penetration = data_.split(',')
